    ...
    int len, int max_pitch, int* pitch)
{
    ...
    int x_lp4_length = len >> 2;
    int y_lp4_length = lag >> 2;
    int xcorr_length = max_pitch >> 1;

    // opus_val16 x_lp4[len >> 2];
    // opus_val16 y_lp4[lag >> 2];
    // opus_val32 xcorr[max_pitch >> 1];
    // opus_val16* x_lp4 = (opus_val16*)malloc(sizeof(opus_val16) * x_lp4_length);
    // opus_val16* y_lp4 = (opus_val16*)malloc(sizeof(opus_val16) * y_lp4_length);
    // opus_val32* xcorr = (opus_val32*)malloc(sizeof(opus_val32) * xcorr_length);
...
static opus_val16 compute_pitch_gain(opus_val32 xy, opus_val32 xx, opus_val32 yy)
{
    return xy / sqrtf(1.0f + xx * yy);
}
#endif

static int second_check[16] = { 0, 0, 3, 2, 3, 2, 5, 2, 3, 2, 3, 2, 5, 2, 3, 2 };
    ...
    int N, int* T0_, int prev_period, opus_val16 prev_gain)
{
    ...
    *T0_ /= 2;
    ...
    // int yy_lookup_length = maxperiod + 1;
    // opus_val32 yy_lookup[maxperiod + 1];
    // opus_val32* yy_lookup = (opus_val32*)malloc(sizeof(opus_val32) * yy_lookup_length);
